...
Charsheet.Generate = async function(msg) {
        ...
        // var crtoken    = findObjs({type: "graphic", represents: crsheet[0].id});
        // log(crtoken);
        
        /*var asyncGetter = async function(character,target){
        	return await new Promise((resolve,reject)=>{
        	    // crtoken = crsheet.forEach(findToken, token);
        	    // crsheet[0].get("_defaulttoken");
        		character.get(target,(details)=>{
        			resolve(details);
		        });
            });
        };
        
        var getToken = async function(character){
            let token = await asyncGetter(character,"_defaulttoken");
            return(token);
        }
        
        var crtoken = await getToken(crsheet[0].id);
        log(crtoken); */
        ...
        function findToken(token, n) {
            token = crsheet[0].get("_defaulttoken");
            log(token);
        }; */
        ...
        //var crgmnotes = filterObjs(function(character) {
        //    if (character.get("gmnotes")) {
        //    return(character.get("gmnotes");}
        //});
